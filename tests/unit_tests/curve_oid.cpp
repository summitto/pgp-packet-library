...
TEST(curve_oid, preset_curves)
{
    ...
    ASSERT_EQ(pgp::curve_oid::ed25519().data(), gsl::span<const uint8_t>(ed25519));
    ASSERT_EQ(pgp::curve_oid::curve_25519().data(), gsl::span<const uint8_t>(curve_25519));
}
...
TEST(curve_oid, other_data)
{
    ...
    ASSERT_EQ(gsl::span<const uint8_t>(data.data() + 1, data.data() + 5), oid.data());

    pgp::curve_oid oid2{data};
    ASSERT_EQ(gsl::span<const uint8_t>(data), oid2.data());
}
